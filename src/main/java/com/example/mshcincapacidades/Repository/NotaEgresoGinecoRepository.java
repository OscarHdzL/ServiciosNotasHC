...
@Repository
public interface NotaEgresoGinecoRepository extends MongoRepository<NotaEgreso, String>{
    
/* NOTAS DE EGRESO PRUEBA */
...
List<NotaEgreso> findNotaEgresoById(String _id);
...
//        "{ $project : { cve_exp_cvesol:1, cve_estatus_expediente:1,  cve_pac_nss : 1 , cve_pac_amedico : 1, cve_pac_curp : 1, fec_nota: 1, des_ooad: 1, des_unidad: 1, cve_especialidad: 1, cve_servicio: 1, cve_nivel: 1, des_diagnostico : 1, des_notas : 1, num_exp_folio: 1, num_exp_anio: 1, num_exp_unidado: 1, num_exp_consec: 1 }}",
        "{ '$match' : {'$and' : [{'cve_pac_nss': ?0},{'cve_pac_amedico':?1}, {'cve_exp_cvesol': '76'},{'cve_estatus_expediente': 'A'}, {'cve_estatus_expediente': 'A'}]}}",
...
List<NotaEgreso> findNotasEgresoById(String id);
...
@Aggregation(
    pipeline = {
        "{'$match':{'$and':[{'cve_pac_nss':?0},{'cve_pac_amedico':?1}, {'cve_exp_cvesol': '76'},{'cve_estatus_expediente': 'A'}, {'cve_exp_cvesol': '76'},{'cve_estatus_expediente': 'A'}]}}",
...
@Aggregation(
    pipeline = {
        "{'$match':{'$and':[{'cve_pac_nss':?0},{'cve_pac_amedico':?1}, {'cve_exp_cvesol': '76'},{'cve_estatus_expediente': 'A'}]}}",
...
@Aggregation(
    pipeline = {
        "{'$match':{'$expr':{'$and':[{'$eq':['$cve_pac_nss',?0]},{'cve_pac_amedico':?1}, {'cve_exp_cvesol': '76'},{'cve_estatus_expediente': 'A'},"+
...
@Aggregation(
    pipeline = {
        "{'$match':{'$expr':{'$and':[{'$eq':['$cve_pac_nss',?0]},{'cve_pac_amedico':?1}, {'cve_exp_cvesol': '76'},{'cve_estatus_expediente': 'A'},{'$regexMatch':{'input':'$des_ooad','regex':?2,'options':'i'}}]}}}",
         "{'$group':{'_id':{'des_unidad':'$des_unidad'},'des_unidad':{'$first': '$des_unidad'}}},{'$sort':{'des_unidad':1}}"
    }
)
...
@Aggregation(
    pipeline = {
        "{'$match':{'$expr':{'$and':[{'$eq':['$cve_pac_nss',?0]},{'cve_pac_amedico':?1}, {'cve_exp_cvesol': '76'},{'cve_estatus_expediente': 'A'},{ '$gte' : [{ '$dateFromString': { 'dateString': '$fec_nota', 'format': '%Y-%m-%d %H:%M:%S'}},?2]},{ '$lt' : [{ '$dateFromString': { 'dateString': '$fec_nota', 'format': '%Y-%m-%d %H:%M:%S'}},?3]},{ '$regexMatch':{'input':'$des_ooad','regex':?4,'options':'i'}}]}}}",
         "{'$group':{'_id':{'des_unidad':'$des_unidad'},'des_unidad':{'$first': '$des_unidad'}}},{'$sort':{'des_unidad':1}}"
    }
)
...
List<Referencia> findNotasReferencia(String num_nss, String agregado_medico, String num_exp_folio, String num_exp_anio, String num_exp_unidado);
...
List<Referencia> findNotasSolicitudServicio(String num_nss, String agregado_medico, String num_exp_folio, String num_exp_anio, String num_exp_unidado);
